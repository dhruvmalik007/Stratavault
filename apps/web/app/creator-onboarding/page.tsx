...
export default function CreatorOnboardingPage() {
  ...
  const primaryWalletAddress = wallet.publicKey;
  ...
  async function fetchPortfolioData() {
    ...
    try {
      const res = await fetch(`${API_BASE}/portfolio`, {
        ...
        body: JSON.stringify({ walletAddress: primaryWalletAddress }),
      });
  ...
  async function onComplete() {
    ...
    try {
      const res = await fetch(`${API_BASE}/creator-profile`, {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          userId: wallet.publicKey, // Use wallet address as ID for now
            ...
            <CardContent className="space-y-3">
              {(['creator', 'participant', 'both'] as const).map((r) => (
                <button
                  key={r}
                  onClick={() => setRole(r)}
                  className={`w-full p-4 border rounded-lg text-left transition-colors ${role === r ? 'bg-primary text-primary-foreground border-primary' : 'hover:bg-accent'
                    }`}
                >
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="p-4 bg-muted rounded-lg">
                ...
                <div className="text-xs text-muted-foreground mt-1 font-mono break-all">{primaryWalletAddress}</div>
              </div>
              <p className="text-sm text-muted-foreground">
                We'll analyze your wallet to verify your experience with DeFi protocols and trading history.
              </p>
              <Button onClick={fetchPortfolioData} disabled={loading || !primaryWalletAddress} className="w-full">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div><strong>Wallet:</strong> <span className="font-mono text-xs">{primaryWalletAddress}</span></div>
              </div>
            </CardContent>
          </Card>
        )}
